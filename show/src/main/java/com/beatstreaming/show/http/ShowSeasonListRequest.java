...
import com.beatstreaming.show.section.ShowSeasonSectionContext;
...
public class ShowSeasonListRequest extends HttpRequestBinding<SeasonEntityList, TitlePageBinding, HomeSectionBinding> {
    ...
    @SneakyThrows
    public ShowSeasonListRequest(Context context, TitlePageBinding titlePageBinding, AppSourceEntity appSourceEntity, TitleEntity titleEntity, HomeSectionBinding homeSectionBinding, ShowSeasonListItemBinder seasonListItemBinder) {
        ...
        this.load(new URIBuilder(this.appSourceEntity.getUrl())
                .setPathSegments("api", "v1", "season", titleEntity.getId())
                .addParameter("type", titleEntity.getType().toString())
                .build());
    }
